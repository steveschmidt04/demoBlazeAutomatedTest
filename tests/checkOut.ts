...
fixture`Checkout/Payment`.page`https://www.demoblaze.com/`
  .before(async (t) => {})
  .beforeEach(async (t) => {
...
test.disablePageCaching(`Verify Payment Model displayed and closed properly if user toggles navigation to it`, async (t) => {
  await t
    .click(hp.headerCart)
    .click(sc.placeOrderBtn)
    .expect(co.modalTitle.innerText)
    .contains("Place order")
    .click(co.xBtn)
    .expect(co.modalTitle.visible)
    .notOk();
  await t
    .click(sc.placeOrderBtn)
    .expect(co.modalTitle.innerText)
...
test.disablePageCaching(`Verify Required fields validated on Checkout Modal if user attempts to submit payment with incomplete information`, async (t) => {
  await t
    .click(hp.headerCart)
    .click(sc.placeOrderBtn)
...
test.disablePageCaching(`Verify Payment able to be complete successfully if user adds items to cart and completes checkout process`, async (t) => {
