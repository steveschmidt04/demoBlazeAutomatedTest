...
fixture`About Us`.page`https://www.demoblaze.com/`
  .before(async (t) => {})
  .beforeEach(async (t) => {
...
test(`Verify About Us modal displayed and closed properly if user toggles navigation from homepage header`, async (t) => {
  await t
    .click(hp.headerAbout)
    .expect(ap.modalTitle.innerText)
    .contains("About us")
    .click(ap.xBtn)
    .expect(ap.video.visible)
    .notOk();
...
test(`Verify About us video plays if user click on video player on About us modal`, async (t) => {
  await t.click(hp.headerAbout).click(ap.video);
