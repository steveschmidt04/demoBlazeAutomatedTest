...
fixture `Sign Up`
    ...
    test(`Verify Sign up modal displayed and closed properly if user toggles navigation from homepage header`, async t => {       
        await t.click(hp.headerSignUp)
               .expect(sp.modalTitle.innerText).contains('Sign up')
               .click(sp.xBtn)
               .expect(sp.userName.visible).notOk()
               .expect(sp.password.visible).notOk()
   ...
   test(`Verify newly created User Account is able to log in`, async t => {  
       ...
       await t.click(hp.headerSignUp)
              ...
              await t.click(hp.headerLogIn) 
                    ...
                    .setNativeDialogHandler(() => true)
                    .click(lp.logInBtn)
                    .expect(hp.headerWelcome.innerText).contains('Welcome ' + randomUser);
   })
